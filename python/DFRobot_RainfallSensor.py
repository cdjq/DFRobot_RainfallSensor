...
class DFRobot_RainfallSensor(object):
  ...
  def get_firmware_version(self):
    ...
    if self._mode == self.I2C_MODE :
      list = self._read_register(self.KIT0192_I2C_REG_VERSION, 2)
      version = list[0] | ( list[1] << 8 )
    else:
      list = self._read_register(self.KIT0192_INPUT_REG_VERSION, 1)
      version = list[0]
  ...
  def get_sensor_working_time(self):
    ...
    '''
    if self._mode == self.I2C_MODE:
      list = self._read_register(self.KIT0192_I2C_REG_SYS_TIME, 2)
      working_time = list[0] | (list[1] << 8)
    else:
      list = self._read_register(self.KIT0192_INPUT_REG_SYS_TIME, 1)
      working_time = list[0]
  ...
  def get_pid_vid(self):
    ...
    ret = False
    if self._mode == self.I2C_MODE:
      list = self._read_register(self.KIT0192_I2C_REG_PID, 4)
      self.pid = list[0] | ( list[1] << 8 ) | ( ( list[3] & 0xC0 ) << 10 )
      self.vid = list[2] | ( ( list[3] & 0x3F ) << 8 )
    else:
      list = self._read_register(self.KIT0192_INPUT_REG_PID, 2)
  ...
  def get_rainfall(self):
    ...
    '''
    if self._mode == self.I2C_MODE:
      list = self._read_register(self.KIT0192_I2C_REG_CUMULATIVE_RAINFALL, 4)
      rainfall = list[0] | ( list[1] << 8 ) | ( list[2] << 16 ) | ( list[3] << 24 )
    else:
      list = self._read_register(self.KIT0192_INPUT_REG_CUMULATIVE_RAINFALL_L, 2)
  ...
  def get_rainfall_time(self,hour):
    ...
    list = [hour]
    if self._mode == self.I2C_MODE:
      self._write_register(self.KIT0192_I2C_REG_RAW_RAIN_HOUR, list)
      list = self._read_register(self.KIT0192_I2C_REG_TIME_RAINFALL, 4)
      rainfall = list[0] | ( list[1] << 8 ) | ( list[2] << 16 ) | ( list[3] << 24 )
    else:
      self._write_register(self.KIT0192_HOLDING_REG_RAW_RAIN_HOUR, list)
      list = self._read_register(self.KIT0192_INPUT_REG_TIME_RAIN_FALL_L, 2)
      rainfall = list[0] | ( list[1] << 16 )
    return rainfall / 10000.0
  ...
  def get_raw_data(self):
    ...
    '''
    if self._mode == self.I2C_MODE:
      list = self._read_register(self.KIT0192_I2C_REG_RAW_DATA, 4)
      raw_data = list[0] | ( list[1] << 8 ) | ( list[2] << 16 ) | ( list[3] << 24 )
    else:
      list = self._read_register( self.KIT0192_INPUT_REG_RAW_DATA_L, 2 )
  ...
  def set_rain_accumulated_value(self, value):
    ...
    if self._mode == self.I2C_MODE:
      ...
      ret=self._write_register(self.KIT0192_I2C_REG_RAW_BASE_RAINFALL, list)
    else:
      ret=self._write_register(self.KIT0192_HOLDING_REG_RAW_BASE_RAINFALL, [data])
    return ret
